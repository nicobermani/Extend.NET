namespace Extend.NET.String
{
    public static partial class StringExtensions
    {
        ...
        {
            int index = @this.IndexOf(oldValue, StringComparison.OrdinalIgnoreCase);
            if (index < 0)
                return @this;
            ...
            while (index >= 0)
            {
                string replacement = PreserveCase(@this.Substring(index, oldValue.Length), newValue);
        ...
        private static string PreserveCase(string original, string replacement)
        {
            ...
            for (int i = 0; i < result.Length && i < original.Length; i++)
            {
